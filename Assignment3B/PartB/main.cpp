...
bool isPressed = true;     // default mouse click
...
//#endif
	window = glfwCreateWindow(g_gl_width, g_gl_height, "Assignment 3B", NULL, NULL);
	if (!window) {
	...
	* SHADERS-------------------------------*/
	GLuint shader_programme =
		InitShader("test_vs.glsl", "test_fs.glsl");
    ...
    Shininess = glGetUniformLocation(shader_programme, "Shininess");
    LightPosition = glGetUniformLocation(shader_programme, "LightPosition");
    AmbientProduct = glGetUniformLocation(shader_programme, "AmbientProduct");
    DiffuseProduct = glGetUniformLocation(shader_programme, "DiffuseProduct");
    SpecularProduct =  glGetUniformLocation(shader_programme, "SpecularProduct");
	...
	while (!glfwWindowShouldClose(window)) {
        ...
        glfwSetKeyCallback(window, mykey);              // key callback
        
        // set light properties
//        glm::vec4 light_ambient(ar, ag, ab, aa);    // R G B A
//        glm::vec4 light_diffuse(dr, dg, db, da);
//        glm::vec4 light_specular(sr, sg, sb, sa);
        
        // set material properties
//        glm::vec4 material_ambient(mat_ar, mat_ag, mat_ab, mat_aa);     // R G B A
//        glm::vec4 material_diffuse(mat_dr, mat_dg, mat_db, mat_da);
//        glm::vec4 material_specular(mat_sr, mat_sg, mat_sb, mat_sa);
    
        // set light position
//        glm::vec4 light_position(pos_x, pos_y, pos_z, pos_w);           // x y z w

		if (GLFW_PRESS == glfwGetKey(window, GLFW_KEY_ESCAPE) || glfwGetKey(window, GLFW_KEY_Q)) {
