...
bool rotation = false;      // set rotation to false
GLfloat position[4] = { 0, 0, 0, 0 };   // set position to 0
GLfloat cosine[3] = { 0 };
...
float offset = 0.005;  // offset
float pi = 3.14159;

// key pressed initial points
int Key_Pressed[maxPoints] = {-1, -1, -1, -1, -1, -1};
//----------------------------------------------------------------------------
...
void init(void)
{
    ...
    positionInt = glGetUniformLocation(program, "position");
    cosineInt = glGetUniformLocation(program, "cosine");
...
//#endif
    window = glfwCreateWindow(640,640, "Rotate Cube", NULL, NULL);
    if (!window) {
